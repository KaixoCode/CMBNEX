...
namespace Kaixo
{
    CombineToggle::CombineToggle(const CRect& size, int index, IControlListener* listener, MyEditor* editor)
        : CViewContainer(size), index(index)
    {
        ...
        {
            addView(new BackgroundEffect{ {.size = { 0,  72, 8,  72 + 67 }, .sides = 1 } });
        ...
        {
            addView(new BackgroundEffect{ {.size = { 0, 144, 8, 144 + 67 }, .sides = 1 } });

            outl = nullptr;
            dest = nullptr;
        }

        addView(new BackgroundEffect{ {.size = {  10 + x, 5,  10 + 140 + x, 5 + 201 }, .dark = true } });
        addView(new BackgroundEffect{ {.size = { 155 + x, 5, 155 + 70 + x, 5 + 201 }, .dark = true } });
        addView(new BackgroundEffect{ {.size = { 230 + x, 5, 230 + 70 + x, 5 + 201 }, .dark = true } });
        addView(new BackgroundEffect{ {.size = { 305 + x, 5, 305 + 70 + x, 5 + 201 }, .dark = true } });
        addView(new BackgroundEffect{ {.size = { 380 + x, 5, 380 + 30 + x, 5 + 201 }, .dark = true } });
        ...
        addView(ftrl); addView(enbr); addView(fltr); addView(freq); addView(reso); addView(minm); 
        addView(mulm); addView(ponm); addView(maxm); addView(modm); addView(andm); addView(inlm); 
        addView(orrm); addView(xorm); addView(addm); addView(drvl); addView(enbd); addView(dgai); 
        addView(driv); addView(fldl); addView(enbf); addView(fold); addView(bias);
    ...
    {
        enbr->forget(); fltr->forget(); freq->forget(); reso->forget(); minm->forget(); mulm->forget(); 
        ponm->forget(); maxm->forget(); modm->forget(); andm->forget(); inlm->forget(); orrm->forget(); 
        xorm->forget(); addm->forget(); enbd->forget(); dgai->forget(); driv->forget(); enbf->forget(); 
       fold->forget(); bias->forget();

        if (dest) dest->forget();
        dcof->forget();
    }

    void CombineView::Select()
    {
        if (!selected)
    ...
    void CombineView::valueChanged(CControl* pControl)
    {
        ...
        toggle->fldl->setDirty(true); toggle->fold->setDirty(true); toggle->bias->setDirty(true);
        toggle->driv->setDirty(true); toggle->drvl->setDirty(true); toggle->dgai->setDirty(true);
        toggle->ftrl->setDirty(true); toggle->fltr->setDirty(true); toggle->freq->setDirty(true);
        toggle->reso->setDirty(true);
    }

    CombineView::CombineView(CombineToggle* toggle, const CRect& size, int index, IControlListener* listener, MyEditor* editor)
        : CViewContainer(size), toggle(toggle), index(index)
    {
        ...
        addView(gain); addView(pgai); addView(gmix);
        addView(titl); addView(inp1); addView(inp2);
    }

    CombineView::~CombineView()
    {
        gain->forget(); pgai->forget(); gmix->forget();
    }

    CMouseEventResult CombinePart::onMouseDown(CPoint& where, const CButtonState& buttons)
    {
    ...
    CombinePart::CombinePart(const CRect& size, IControlListener* listener, MyEditor* editor)
        : CViewContainer(size)
    {
        setBackgroundColor({ 0, 0, 0, 0 });
